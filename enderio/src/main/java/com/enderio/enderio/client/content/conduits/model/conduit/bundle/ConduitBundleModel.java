package com.enderio.enderio.client.content.conduits.model.conduit.bundle;

import static com.enderio.legacy_layout.conduits.client.ConduitClientSetup.*;

import com.enderio.enderio.api.conduits.Conduit;
import com.enderio.enderio.api.conduits.model.ConduitModelModifier;
import com.enderio.enderio.common.content.conduits.utility.ConduitSorter;
import com.enderio.enderio.client.content.conduits.model.BoxTextureQuadTransformer;
import com.enderio.enderio.client.content.conduits.model.ColorQuadTransformer;
import com.enderio.enderio.client.content.conduits.model.ConduitTextureEmissiveQuadTransformer;
import com.enderio.enderio.client.content.conduits.model.conduit.facades.ClientFacadeVisibility;
import com.enderio.enderio.client.content.conduits.model.conduit.modifier.ConduitModelModifiers;
import com.enderio.legacy_layout.conduits.common.Area;
        ...
        if (bundleState != null) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                for (int i = 0; i < connectedTypes.size(); i++) {
                    ...
                    Vec3i offset = ConduitSorter.OffsetHelper.translationFor(direction.getAxis(),
                            ConduitSorter.OffsetHelper.offsetConduit(i, connectedTypes.size()));
...
//                                                extraData, renderType)));
            ...
            if (box != null) {
                for (Map.Entry<Holder<Conduit<?, ?>>, Integer> notRenderedEntry : notRendered.entrySet()) {
                    Vec3i offset = ConduitSorter.OffsetHelper.translationFor(axis,
                            ConduitSorter.OffsetHelper.offsetConduit(notRenderedEntry.getValue(), allTypes.size()));
                ...
                for (Map.Entry<Holder<Conduit<?, ?>>, Integer> notRenderedEntry : notRendered.entrySet()) {
                    quads.addAll(new ConduitTextureEmissiveQuadTransformer(
                            sprite(bundleState.getTexture(notRenderedEntry.getKey())), 0).andThen(
                                    QuadTransformers.applying(translateTransformation(ConduitSorter.OffsetHelper.translationFor(axis,
                                            ConduitSorter.OffsetHelper.offsetConduit(notRenderedEntry.getValue(), allTypes.size())))))
                                    .process(modelOf(CONDUIT_CORE).getQuads(state, side, rand, extraData, renderType)));
                }
            }
