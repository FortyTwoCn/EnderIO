...
public record OutputStack(Either<ItemStack, FluidStack> stack) {
    ...
    public CompoundTag serializeNBT(HolderLookup.Provider lookupProvider) {
        ...
        if (isItem()) {
            tag.put(EIONBTKeys.ITEM, stack.left().get().saveOptional(lookupProvider));
        } else if (isFluid()) {
            tag.put(EIONBTKeys.FLUID, stack.right().get().saveOptional(lookupProvider));
        }
        return tag;
    }
    ...
    public static OutputStack fromNBT(HolderLookup.Provider lookupProvider, CompoundTag tag) {
        if (tag.contains(EIONBTKeys.ITEM)) {
            return OutputStack.of(ItemStack.parseOptional(lookupProvider, tag.getCompound(EIONBTKeys.ITEM)));
        } else if (tag.contains(EIONBTKeys.FLUID)) {
            return OutputStack.of(FluidStack.parseOptional(lookupProvider, tag.getCompound(EIONBTKeys.FLUID)));
        }
