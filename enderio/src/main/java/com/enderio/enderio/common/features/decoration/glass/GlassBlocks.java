package com.enderio.enderio.common.features.decoration.glass;
...
public class GlassBlocks {
    ...
    private RegiliteBlock<FusedQuartzBlock> register(BlockRegistry blockRegistry, ItemRegistry itemRegistry, String name) {
        var block = blockRegistry
            ...
            .addBlockTags(
                glassIdentifier.explosion_resistance() ? EnderIOTags.Blocks.FUSED_QUARTZ : EnderIOTags.Blocks.CLEAR_GLASS
            );

        block.createBlockItem(itemRegistry, item -> {
            item.setTab(EIOCreativeTabs.BLOCKS)
                .addItemTags(
                    glassIdentifier.explosion_resistance() ? EnderIOTags.Items.FUSED_QUARTZ : EnderIOTags.Items.CLEAR_GLASS,
                    EnderIOTags.Items.GLASS_TAGS.get(glassIdentifier)
                )
            ...
            if (glassIdentifier.lighting() == GlassLighting.EMITTING && glassIdentifier.explosion_resistance()) {
                item.addItemTags(EnderIOTags.Items.ENLIGHTENED_FUSED_QUARTZ);
            }

            if (glassIdentifier.lighting() == GlassLighting.BLOCKING && glassIdentifier.explosion_resistance()) {
                item.addItemTags(EnderIOTags.Items.DARK_FUSED_QUARTZ);
            }
        });
