package com.enderio.enderio.common.content.machines.blocks.powered_spawner;
...
import com.enderio.legacy_layout.base.common.particle.RangeParticleData;
...
import com.enderio.enderio.common.content.machines.base.inventory.SingleSlotAccess;
...
import com.enderio.enderio.common.content.machines.base.task.MachineTask;
import com.enderio.enderio.common.content.machines.base.task.host.MachineTaskHost;
...
public class PoweredSpawnerBlockEntity extends PoweredMachineBlockEntity implements IOwnedSpawner, ISoulBindable {
    ...
    protected void saveAdditionalSynced(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        // future :)
        tag.put(EIONBTKeys.ENTITY_STORAGE, boundSoul.saveOptional(registries));

        if (mode != DEFAULT_MODE) {
            tag.put(EIONBTKeys.MACHINE_MODE, mode.save(registries));
        }

        if (isRangeVisible) {
            tag.putBoolean(EIONBTKeys.IS_RANGE_VISIBLE, isRangeVisible);
        }
    }
    ...
    public void loadAdditional(CompoundTag pTag, HolderLookup.Provider lookupProvider) {
        ...
        boundSoul = Soul.parseOptional(lookupProvider, pTag.getCompound(EIONBTKeys.ENTITY_STORAGE));

        if (pTag.contains(EIONBTKeys.MACHINE_MODE)) {
            this.mode = PoweredSpawnerMode.parse(lookupProvider,
                    Objects.requireNonNull(pTag.get(EIONBTKeys.MACHINE_MODE)));
        }
        ...
        isRangeVisible = pTag.contains(EIONBTKeys.IS_RANGE_VISIBLE)
                && pTag.getBoolean(EIONBTKeys.IS_RANGE_VISIBLE);
    ...
    public void removeComponentsFromTag(CompoundTag tag) {
        ...
        tag.remove(EIONBTKeys.IS_RANGE_VISIBLE);
