...
public class GlassRecipeProvider extends RecipeProvider {
    ...
    public static Map<Item, GlassCollisionPredicate> TOKEN_TO_PREDICATE = Map.ofEntries(
        Map.entry(EIOItems.PLAYER_TOKEN.get(), GlassCollisionPredicate.PLAYERS_PASS),
        Map.entry(EIOItems.ANIMAL_TOKEN.get(), GlassCollisionPredicate.ANIMALS_PASS),
        Map.entry(EIOItems.MONSTER_TOKEN.get(), GlassCollisionPredicate.MOBS_PASS)
    );
    ...
    private static void recolor(GlassBlocks blocks, RecipeOutput recipeOutput) {
        for (DyeColor color: DyeColor.values()) {
            ...
            for (int i = 0; i < 8; i++) {
                builder.requires(EnderIOTags.Items.GLASS_TAGS.get(blocks.getGlassIdentifier()));
            }
    ...
    private static void addCollisionToken(GlassBlocks blocks, Item token, RecipeOutput recipeOutput) {
        if (!TOKEN_TO_PREDICATE.containsKey(token)) {
            return;
        }

        GlassCollisionPredicate collision = TOKEN_TO_PREDICATE.get(token);
